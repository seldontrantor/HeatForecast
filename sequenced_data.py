...
class SequencedData():
    ...
    def make_dataset(self, data):
        ...
        if self.SHUFFLE:
            ds = ds.shuffle(buffer_size=self.buffer_length, seed=42, reshuffle_each_iteration=False)
        return ds
    ...
    def train(self):
        ...
        return self.make_dataset(self.train_df).map(self.split_windows_map).map(self.transform_element).prefetch(tf.data.AUTOTUNE)

    def val(self):
        """Returns the validation dataset."""
        return self.make_dataset(self.val_df).map(self.split_windows_map).map(self.transform_element).prefetch(tf.data.AUTOTUNE)

    def test(self):
        """Returns the test dataset."""
        self.SHUFFLE = False
        return self.make_dataset(self.test_df).map(self.split_windows_map).map(self.transform_element).prefetch(tf.data.AUTOTUNE)
    ...
    def plot(self, inputs, labels, title, predictions=None, plot_col='Demand', max_subplots=3):
        ...
        for n in range(max_n):
            ...
            plt.plot(np.arange(self.input_width)[self.input_slice], inputs[n, :, plot_col_index],
                     label='Inputs', marker='.', zorder=-10)
            label_col_index = self.label_columns_indices.get(plot_col)
            plt.scatter(np.arange(self.total_window_size)[self.labels_slice], labels[n, :],
                        edgecolors='k', label='Labels', c='#2ca02c', s=64)
            if predictions is not None:
                plt.scatter(np.arange(self.total_window_size)[self.labels_slice], predictions[n, :, label_col_index],
