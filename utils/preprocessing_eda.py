...
def add_holiday_weekend_features(df, country_code="DK"):
    ...
    df["Holidays"] = (
        df.index.to_series().apply(lambda x: x in holiday_dates).astype(int)
    )
...
def plot_daily_trends(df):
    pivot_table = df.pivot_table(
        index="hour", columns="weekday", values="Demand", aggfunc="sum"
    )
    fig, ax = plt.subplots()
...
def plot_quarterly_boxplots(df):
    for q in range(1, 5):
        ...
        sns.boxplot(
            x=df[df["quarter"] == q]["hour"], y=df[df["quarter"] == q]["Demand"], ax=ax
        )
