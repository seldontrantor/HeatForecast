...
from sequenced_data import windowed_train, windowed_test, windowed_val, input_shape, output_shape, BATCH_SIZE
...
def main():
    ...
    parser.add_argument(
        "model_path", help="Path to the saved model or weights file."
    )
    parser.add_argument(
        "--weights",
        action="store_true",
        help="Interpret `model_path` as a weights file instead of a full model.",
    )
    args = parser.parse_args()

    if args.weights:
        model = load_trained_model(args.model_path, input_shape, output_shape)
    else:
    ...
    plt.title(
        f"{args.model_path[-38:-18]} and R^2: {np.round(r2,4)}"
    )
