...
from utils.lstm_utils import evaluate_model, plot_training_history, save_model_artifacts
...
def define_models_dense(input_shape, output_shape, n_units=n_units):
    ...
    decoder_outputs = Dense(1, activation="relu")(decoder_outputs)
    decoder_model = Model(
        [decoder_inputs] + decoder_states_inputs, [decoder_outputs, state_h, state_c]
    )
...
def main(nepoch, learning_rate):
    ...
    print(
        f"Training model with input shape {input_shape} and output shape of {output_shape}"
    )
    ...
    callbacks_list = [
        TensorBoard(
            log_dir=os.path.join("logs", run_time),
            histogram_freq=1,
            profile_batch="10,30",
        ),
        ModelCheckpoint(
